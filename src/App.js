...
function App() {
  ...
  const handleRemoveFollowingClick = (followingValue) => {
    const followingArray = JSON.parse(localStorage.getItem("following"));
    let newArray = followingArray.filter(
      (robotName) => robotName !== followingValue
    );
    localStorage.setItem("following", JSON.stringify(newArray));
  };
  return (
